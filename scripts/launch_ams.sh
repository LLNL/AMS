...
RMQ_TMP="rmq.json"
CERT_TLS="rmq.cert"
# TODO: create a specific queue for the AMS daemon to listen to
AMS_DAEMON_QUEUE="test3"
...
if [[ $USE_DB && $DBTYPE = "rmq" ]]; then
  ...
  REMOTE_HOST=$(jq ".rabbitmq.service-host" $AMS_JSON)
  REMOTE_PORT=$(jq ".rabbitmq.service-port" $AMS_JSON)
