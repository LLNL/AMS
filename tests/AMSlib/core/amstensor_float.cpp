                ...
                "[ams][tensor][float][view]")
{
  ...
  auto v = AMSTensor::view<float>(data.data(),
                                  shape,
                                  strides,
...
CATCH_TEST_CASE("float: write and read 1D data", "[ams][tensor][float][data]")
{
  AMSInit();
  std::vector<AMSTensor::IntDimType> shape = {4};
  std::vector<AMSTensor::IntDimType> strides = {1};
...
CATCH_TEST_CASE("float: write and read 2D data", "[ams][tensor][float][data]")
{
  AMSInit();
  std::vector<AMSTensor::IntDimType> shape = {3, 4};
  std::vector<AMSTensor::IntDimType> strides = {4, 1};
...
CATCH_TEST_CASE("float: transpose 2D tensor", "[ams][tensor][float][transpose]")
{
  AMSInit();
...
CATCH_TEST_CASE("float: move assignment", "[ams][tensor][float][move]")
{
  ...
  auto t1 =
      AMSTensor::create<float>(shape1, strides, AMSResourceType::AMS_HOST);
  auto t2 =
      AMSTensor::create<float>(shape2, strides, AMSResourceType::AMS_HOST);
                ...
                "[ams][tensor][float][clone]")
{
  ...
  auto view = AMSTensor::view<float>(src.data(),
                                     shape,
                                     strides,
                                     AMSResourceType::AMS_HOST);
                ...
                "[ams][tensor][float][clone][transpose]")
{
  ...
  auto original = AMSTensor::view<float>(src.data(),
                                         shape,
                                         strides,
                                         AMSResourceType::AMS_HOST);
...
CATCH_TEST_CASE("float: concat single tensor", "[ams][tensor][float][concat]")
{
  AMSInit();
...
CATCH_TEST_CASE("float: concat two 2D tensors", "[ams][tensor][float][concat]")
{
  AMSInit();
  // A:[3,2]       B:[3,3]
  // [1, 2]        [7, 8, 9]
  ...
  std::vector<float> expected = {
      1, 2, 7, 8, 9, 3, 4, 10, 11, 12, 5, 6, 13, 14, 15};
  for (int i = 0; i < 15; ++i) {
    CATCH_INFO("index " << i);
...
CATCH_TEST_CASE("float: concat three tensors", "[ams][tensor][float][concat]")
{
  AMSInit();
  ...
  auto tB = AMSTensor::view<float>(b.data(),
                                   ...
                                   stridesB,
                                   AMSResourceType::AMS_HOST);
  auto tC = AMSTensor::view<float>(c.data(),
                                   shapeC,
                                   stridesC,
                                   AMSResourceType::AMS_HOST);
                ...
                "[ams][tensor][double][view]")
{
  ...
  auto v = AMSTensor::view<double>(data.data(),
                                   shape,
                                   strides,
                                   AMSResourceType::AMS_HOST);
...
CATCH_TEST_CASE("double: write and read data", "[ams][tensor][double][data]")
{
  AMSInit();
                ...
                "[ams][tensor][double][clone]")
{
  ...
  auto view = AMSTensor::view<double>(src.data(),
                                      shape,
                                      strides,
                                      AMSResourceType::AMS_HOST);
  auto cloned = view.clone();
                ...
                "[ams][tensor][double][clone][transpose]")
{
  ...
  auto original = AMSTensor::view<double>(src.data(),
                                          shape,
                                          strides,
                                          AMSResourceType::AMS_HOST);
  auto transposed = original.transpose(0, 1);
...
CATCH_TEST_CASE("double: concat 1D tensors", "[ams][tensor][double][concat]")
{
  AMSInit();
