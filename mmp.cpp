...
int eval_data(int stop_cycle, bool is_cpu, bool pack_sparse_mats, int num_qpts, int num_elems, int num_mats,
              double *density_in, double *energy_in,
              bool *indicators
              )
{
   // inputs
   ...
   for (int c = 0; c <= stop_cycle; ++c)
   {
      ...
      for (int k = 0; k < num_mats; ++k)
      {
         ...
         for (int i = 0; i < num_elems; ++i)
         {
            num_elems_k += indicators[i+k*num_elems];
         }
...
int main(int argc, char **argv)
{
   ...
   // setup indicators
   bool indicators_arr[num_elems * num_mats];
   for (int k = 0; k < num_mats; ++k)
   {
   ...
   // inputs
   double density[num_qpts*num_elems*num_mats];
   double energy[num_qpts*num_elems*num_mats];
   ...
   for (int k = 0; k < num_mats; ++k)
   {
      for (int i = 0; i < num_elems; ++i)
      {
         if (!indicators_arr[i+k*num_elems])
         {
            continue;
         }
         for (int j = 0; j < num_qpts; ++j)
         {
            density[j+ i*num_qpts + k*num_elems*num_qpts] = .1 + unitrand();
            energy[j+ i*num_qpts + k*num_elems*num_qpts] = .1 + unitrand();
         }
      }
   }
   eval_data(stop_cycle, is_cpu, pack_sparse_mats, num_qpts, num_elems, num_mats, density, energy, indicators_arr);
   return 0;
}
