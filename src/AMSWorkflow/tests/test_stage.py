...
class TestStage(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ams_config = AMSInstance(config=tmp_dict)
    ...
    def verify(self, data, reader):
        ...
        with store.AMSDataStore(ams_config.db_path, ams_config.db_store, ams_config.name, False) as fd:
            versions = fd.get_candidates_versions(True)
            r_inputs = list()
            r_outputs = list()
            files = list()
            ...
            self.assertTrue( np.array_equal(pipe_in_data, origin_in_data), "inputs do not match after writting them with pipeline")

            pipe_out_data = np.concatenate(r_outputs, axis=0)
            origin_out_data = np.concatenate( [ d[1] for d in data ], axis = 0)
            self.assertTrue( np.array_equal(pipe_out_data, origin_out_data), "outputs do not match after writting them with pipeline")
    ...
    def test_fs_pipeline(self):
        ...
        for wr in stage.Pipeline.supported_writers:
            ...
            for p in stage.Pipeline.supported_policies:
                ...
                pipe = stage.FSPipeline(True, self.o_dir,  None, writer.get_file_format_suffix(), self.i_dir,
                                        writer.get_file_format_suffix(), "*.{0}".format(writer.get_file_format_suffix()))
                with timeout(10, error_message="Copying files from {0} to {1} using writer {2} and policy {3}".format(self.i_dir,
                                                                                                                      self.o_dir,
                                                                                                                      wr, p)):
                        pipe.execute(p)
