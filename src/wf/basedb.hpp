...
public:
  ...
  /**
   * Takes an input and an output vector each holding 1-D vectors data, and store.
   * them in persistent data storage.
   ...
   * @param[in] inputs Vector of 1-D vectors, each 1-D vectors contains 'num_elements'  values to be stored
   * @param[in] outputs Vector of 1-D vectors, each 1-D vectors contains 'num_elements'  values to be stored
   */
...
public:
   ...
   * */
  FileDB(std::string path, const std::string suffix, uint64_t rId) : BaseDB<TypeValue>(rId) {
    fs::path Path(path);
    std::error_code ec;
...
public:
  ...
  /**
   * Takes an input and an output vector each holding 1-D vectors data, and store them into a csv file delimited by ':'.
   * This should never be used for large scale simulations as txt/csv format will be extremely slow.
   * @param[in] num_elements Number of elements of each 1-D vector
   * @param[in] inputs Vector of 1-D vectors containing the inputs to bestored
   * @param[in] inputs Vector of 1-D vectors, each 1-D vectors contains 'num_elements'  values to be stored
   * @param[in] outputs Vector of 1-D vectors, each 1-D vectors contains 'num_elements'  values to be stored
...
private:
   ...
   * storing data as Ckunked pieces. The Chunk value controls the chunking performed
   * by HDF5 and thus controls the write performance
   ...
   */
  hid_t getDataSet(hid_t group, std::string dName, const size_t Chunk = 32L * 1024L * 1024L) {
    ...
    if ( exists > 0 ){
      ...
      // TODO: Align this with the caching mechanism for this option to work out.
      hsize_t cDims = Chunk;
      H5Pset_chunk(pList, nDims, &cDims);
      dset = H5Dcreate(group, dName.c_str(), HDType, fileSpace, H5P_DEFAULT,
                       pList, H5P_DEFAULT);
      HDF5_ERROR(dset);
      H5Sclose(fileSpace);
      H5Pclose(pList);
  ...
  /**
   * @brief Create the HDF5 datasets and store their descriptors in the in/out vectors
   ...
   */
  void createDataSets(size_t numElements, const size_t numIn, const size_t numOut){
    ...
    for (int i = 0; i < numOut; i++){
      hid_t dSet = getDataSet(HFile, std::string("output_") + std::to_string(i));
   ...
   * @param[in] dsets Vector containing the hdf5-dataset descriptor for every vector to be written
   * @param[in] data vectors containing 1-D vectors of numElements values each to be written in the db.
   * @param[in] numElements The number of elements each vector has
   */
  void writeDataToDataset(std::vector<hid_t>& dsets, std::vector<TypeValue*>& data, size_t numElements){
    int index = 0;
    for ( auto *I : data){
...
public:
   ...
   * This should never be used for large scale simulations as txt/hdf5 format will be extremely slow.
   * @param[in] num_elements Number of elements of each 1-D vector
   * @param[in] inputs Vector of 1-D vectors containing the inputs to bestored
   * @param[in] inputs Vector of 1-D vectors, each 1-D vectors contains 'num_elements'  values to be stored
   * @param[in] outputs Vector of 1-D vectors, each 1-D vectors contains 'num_elements'  values to be stored
  ...
  virtual void store(size_t num_elements,
    ...
    if ( HDIsets.size() != num_in || HDOsets.size() != num_out ){
      std::cerr << "The data dimensionality is different than the one in the DB\n";
      exit(-1);
    }
...
public:
     ...
     * @param[in] rId a unique Id for each process taking part in a distributed execution (rank-id)
     */
    RedisDB(std::string fn, uint64_t rId) : BaseDB<TypeValue>(rId),  _fn(fn), _redis(nullptr), keyId(0) {
    ...
    inline std::string type() override {
      return "RedisDB";
    }
    ...
    void store(size_t num_elements,
      ...
      for (size_t i = 0; i < num_elements; i++) {
        std::string key = std::to_string(_dbid)+":"+std::to_string(keyId)+":"+std::to_string(i); // In Redis a key must be a string
...
#ifdef __ENABLE_DB__
  if (dbPath == nullptr){
    std::cerr<< " [WARNING] Path of DB is NULL, Please provide a valid path to enable db\n";
